...
import { exportCasePDF, exportCaseWord } from "./utils/exporter";
...
export default function App() {
  ...
  const triggerExportMatrix = (format: "csv" | "json") => {
    ...
    // Memory efficient mock stream trigger to download
    if (format === "csv") {
      const headers = [
        "Case ID", "Case Index No", "Party Name", "Party Status", 
        "Advocate", "Category", "Judicial Forum", "Case Type", "Filing Year", 
        "Status", "Note", "Filing Start", "Filing End", 
        "Hearing Start", "Hearing End", "Hearing Progression Stage"
      ];
      const rows = target.map(row => [
        row.id, 
        `"${row.caseIndexNo.replace(/"/g, '""')}"`, 
        `"${row.petitionerParty.replace(/"/g, '""')}"`, 
        `"${row.respondentParty.replace(/"/g, '""')}"`,
        `"${row.advocateOnRecord.replace(/"/g, '""')}"`, 
        row.classificationCategory, 
        `"${row.judicialForum.replace(/"/g, '""')}"`,
        `"${row.writCaseType.replace(/"/g, '""')}"`, 
        row.filingYearTarget, 
        row.currentCaseStatus,
        `"${row.keywordsContentMapping.replace(/"/g, '""')}"`, 
        row.filingDateStart, 
        row.filingDateEnd,
        row.hearingDateStart, 
        row.hearingDateEnd, 
        `"${getOrdinalBadgeText(row.hearingIndex)}"`
      ]);
      ...
      link.setAttribute("download", `veritas_batch_stream_${Date.now()}.csv`);
      ...
      const link = document.createElement("a");
      link.setAttribute("href", url);
      link.setAttribute("download", `veritas_batch_stream_${Date.now()}.json`);
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
    }
    ...
    <div className="min-h-screen bg-transparent text-[#1A1A1A] font-sans selection:bg-lavender selection:text-slate-900">
      ...
      <AnimatePresence>
        {toast && (
          ...
          >
            <div className="w-2.5 h-2.5 rounded-full bg-mint animate-pulse border border-emerald-400" />
                  ...
                  <div className="lg:col-span-8 space-y-6">
                        ...
                        >
                          <div className="bg-white/95 border border-mauve rounded-xl p-6 shadow-sm space-y-4">
                            ...
                            <p className="text-[11px] text-slate-500 font-mono leading-relaxed">
                              Customize query variables below. The export matrices compiled dynamically are generated using a simulated memory-efficient stream mapping perfectly to our FastAPI <code>/api/export</code> streaming buffer pipeline.
                            </p>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 pt-2">
                              ...
                              <div className="flex flex-col justify-end">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                  <button
                                    id="btn-trigger-csv"
                                    onClick={() => triggerExportMatrix("csv")}
                                  ...
                                  <button
                                    id="btn-trigger-json"
                                    onClick={() => triggerExportMatrix("json")}
                    ...
                    <div className="bg-white/95 border border-mauve rounded-xl shadow-sm overflow-hidden">
                      ...
                      {filteredCases.length === 0 ? (
                        <div className="p-16 text-center space-y-4 bg-slate-50/10">
                          <BookOpen className="w-10 h-10 text-lavender mx-auto" />
                          <h4 className="font-mono text-sm text-slate-700 font-bold uppercase">
                          ...
                          <button
                            onClick={() => setSearchQuery("")}
                            className="text-xs font-mono font-bold text-slate-700 hover:text-white hover:bg-slate-800 bg-white border border-slate-200 px-4 py-2 rounded-lg transition-all shadow-sm cursor-pointer"
                          >
                                    ...
                                    <td className="p-4">
                                      <div className="flex items-center gap-1.5 flex-wrap">
                                        <span className={`text-[10px] font-mono uppercase tracking-wider px-2.5 py-1 rounded-xl font-bold border ${
                                          ...
                                          c.classificationCategory === "Civil" ? "bg-mint/45 text-slate-805 border-emerald-300" :
                                        ...
                                        <button
                                          ...
                                          className={`flex items-center gap-1 px-3 py-1 rounded-full font-bold font-mono text-[9px] uppercase tracking-wider border cursor-pointer transition-all hover:scale-105 active:scale-95 ${
                                            c.currentCaseStatus === "Completed"
                                              ? "bg-mint text-slate-800 border-emerald-300 shadow-3xs"
                                    ...
                                    <td className="p-4 text-right whitespace-nowrap">
                                      <span className={`font-mono text-xs font-bold border px-3 py-1.5 rounded-full inline-flex items-center gap-1.5 ${
                                        upcomingHearing
                                          ...
                                          : "bg-mint/45 border-emerald-300 text-slate-805"
                                      }`}>
                                        <div className={`w-2 h-2 rounded-full ${
                                          upcomingHearing ? "bg-bubblegum animate-pulse" : "bg-emerald-500"
                                        }`} />
                                        <span>
                                          {upcomingHearing 
                  ...
                  <div className="bg-slate-50 border border-mauve/60 rounded-xl p-5 space-y-4">
                    ...
                    {(() => {
                      ...
                      if (upcoming) {
                        ...
                        return (
                          <div className="bg-white border border-dashed border-emerald-300 rounded-lg p-6 text-center text-xs font-mono text-emerald-700">
