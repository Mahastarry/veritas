...
// Batch PDF Export using jsPDF with crisp clean layout for list of dockets
export const exportBatchPDF = (cases: LegalCase[]) => {
  const doc = new jsPDF({
    orientation: "portrait",
    unit: "mm",
    format: "a4",
  ...
  const drawFooter = () => {
    ...
    doc.setFont("helvetica", "normal");
    doc.setFontSize(8);
    doc.text("VERITAS LEGAL BATCH SYSTEM", 20, 286);
    doc.text("Confidential Export Summary", 190, 286, { align: "right" });
  };
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(20);
  doc.text("VERITAS BATCH STATUS REPORT", 20, y);
  
  doc.setTextColor(100, 100, 100);
  doc.setFontSize(9.5);
  doc.setFont("helvetica", "normal");
  y += 5;
  doc.text(`Total Records: ${cases.length} Case Dockets`, 20, y);
  
  doc.setTextColor(140, 140, 140);
  doc.setFontSize(8);
  doc.text(`Export Date: ${new Date().toISOString().replace('T', ' ').substring(0, 10)}`, 190, y, { align: "right" });
  ...
  cases.forEach((c, index) => {
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(11);
    doc.text(`${index + 1}. CASE: ${c.caseIndexNo}`, 20, y);
    y += 6;
    ...
    const info = [
      `Petitioner: ${c.petitionerParty}`,
      `Respondent: ${c.respondentParty}`,
      `Advocate: ${c.advocateOnRecord || "N/A"}`,
      `Forum: ${c.judicialForum}`,
      `Type: ${c.classificationCategory} - ${c.writCaseType}`,
      `Status: ${c.currentCaseStatus}`,
    ];
    ...
    if (c.caseSummary) {
      ...
      doc.setTextColor(100, 100, 100);
      const summaryLines = doc.splitTextToSize(`Summary: ${c.caseSummary}`, 160);
      summaryLines.forEach((line: string) => {
        checkPageBreak(5);
        doc.text(line, 25, y);
        y += 4.2;
  ...
  doc.save(`Veritas_Batch_Export_${Date.now()}.pdf`);
};

// Batch DOCX/Word Export using docx with clean structures
export const exportBatchWord = (cases: LegalCase[]) => {
  const doc = new Document({
    sections: [
      {
        properties: {},
        children: [
          new Paragraph({
            ...
            children: [
              new TextRun({
                text: "VERITAS BATCH STATUS REPORT",
                bold: true,
                size: 32,
                color: "111111",
              }),
          ...
          new Paragraph({
            ...
            children: [
              new TextRun({
                text: `Exported Records: ${cases.length} Case Dockets | Date: ${new Date().toISOString().substring(0, 10)}`,
                color: "666666",
                bold: true,
                size: 14,
              }),
          ...
          ...cases.flatMap((c, index) => {
            ...
            return [
              new Paragraph({
                children: [
                  new TextRun({
                    text: `${index + 1}. CASE INDEX: ${c.caseIndexNo}`,
                    bold: true,
                    size: 20,
                    color: "1A1A1A",
                  }),
              ...
              new Paragraph({
                children: [
                  new TextRun({ text: "• Petitioner Party: ", bold: true }),
                  new TextRun({ text: c.petitionerParty }),
                ],
              }),
              new Paragraph({
                children: [
                  new TextRun({ text: "• Respondent Party: ", bold: true }),
                  new TextRun({ text: c.respondentParty }),
                ],
              }),
              new Paragraph({
                children: [
                  new TextRun({ text: "• Advocate On Record: ", bold: true }),
                  new TextRun({ text: c.advocateOnRecord || "N/A" }),
                ],
              }),
              new Paragraph({
                children: [
                  new TextRun({ text: "• Court / Judicial Forum: ", bold: true }),
                  new TextRun({ text: c.judicialForum }),
                ],
              }),
              new Paragraph({
                children: [
                  new TextRun({ text: "• Classification: ", bold: true }),
                  new TextRun({ text: `${c.classificationCategory} - ${c.writCaseType}` }),
                ],
              }),
              new Paragraph({
                children: [
                  new TextRun({ text: "• Case Status: ", bold: true }),
                  new TextRun({ text: c.currentCaseStatus }),
                ],
              }),
              new Paragraph({
                children: [
                  new TextRun({ text: "• Upcoming Scheduled Hearing: ", bold: true }),
                  new TextRun({ text: upcomingText }),
                ],
              }),
              new Paragraph({
                children: [
                  new TextRun({ text: "• Summary: ", bold: true, italics: true }),
                  new TextRun({ text: c.caseSummary || "N/A", italics: true }),
                ],
              }),
              new Paragraph({ text: "" }),
              new Paragraph({
          ...
          new Paragraph({
            ...
            children: [
              new TextRun({
                text: "END OF VERITAS BATCH EXPORT SUMMARY",
                size: 14,
                color: "999999",
                bold: true,
              }),
  ...
  Packer.toBlob(doc).then((blob) => {
    ...
    const link = document.createElement("a");
    link.setAttribute("href", url);
    link.setAttribute("download", `Veritas_Batch_Export_${Date.now()}.docx`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  });
